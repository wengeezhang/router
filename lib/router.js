...
Router.prototype.match = function (path, method) {
  const matchMode = this.opts.matchMode || 'regexp';
  ...
  for (let len = layers.length, i = 0; i < len; i++) {
    ...
    // if user chooses "strict-equality" mode, 
    // then we'll get matched route directly by strict equality(===)
    // koa-router leverages 'strict-equality' mode to gain performance rise
    if (matchMode === 'strict-equality'? (layer.path === '(.*)' || layer.path === path) : layer.match(path)) {
